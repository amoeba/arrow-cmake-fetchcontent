...
arrow::Status RunMain() {
  ...
  // The Datum class is what all compute functions output to, and they can take
  // Datums as inputs, as well.
  arrow::Datum sum;
  // (Doc section: Sum Datum Declaration)
  // (Doc section: Sum Call)
  // Here, we can use arrow::compute::Sum. This is a convenience function, and
  // the next computation won't be so simple. However, using these where
  // possible helps readability.
  ...
  // Note that we explicitly request a scalar -- the Datum cannot simply give
  // what it is, you must ask for the correct type.
  ...
  // Get element-wise sum of both columns A and B in our Table. Note that here
  // we use CallFunction(), which takes the name of the function as the first
  // argument.
  ...
  // Get the kind of Datum and what it holds -- this is a ChunkedArray, with
  // int32.
  ...
  // Use an options struct to set up searching for 2223 in column A (the third
  // item).
  arrow::Datum third_item;
  // (Doc section: Index Datum Declare)
  // (Doc section: IndexOptions Declare)
  // An options struct is used in lieu of passing an arbitrary amount of
  // arguments.
  ...
  // We get a scalar -- the location of 2223 in column A, which is 2 in 0-based
  // indexing.
